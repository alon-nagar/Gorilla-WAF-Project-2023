...
app = Flask(__name__)
db = gui_database.MongoDB("mongo", 27017)  # Alon's IP: 172.17.0.2
...
def handle_get_all_allowed_redirect_urls():
    return jsonify(db.get_all_allowed_urls())
...
def handle_remove_url_from_allowed_redirect_urls():
    try:
        ...
        url.strip()
        if url != "":
            return db.delete_allowed_url(url)
        return "URL can't be empty"
